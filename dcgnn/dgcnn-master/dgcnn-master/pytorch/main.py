...
train_accuracies = []
test_accuracies = []
def dynamic_line(epoch, total_epochs, step, total_steps, train_loss, train_acc):
    """
    Display a dynamic updating line in the console.
    """
    progress = int((step / total_steps) * 20)  # 进度条长度，最多20格
    bar = '-' * progress + ' ' * (20 - progress)  # 构造进度条
    sys.stdout.write(f'\rEpoch [{epoch}/{total_epochs}] '
...
def train(args, io):
    ...
    for epoch in range(args.epochs):
        ...
        outstr = f"Train {epoch}, loss: {train_loss / count:.6f}, acc: {train_acc:.6f}, avg acc: {avg_train_acc:.6f}"
        io.cprint(outstr)
        ...
        outstr = f"Test {epoch}, loss: {test_loss / count:.6f}, acc: {test_acc:.6f}, avg acc: {avg_test_acc:.6f}"
        io.cprint(outstr)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--prune_threshold', type=float, default=0.01,
                        help='Pruning threshold for weight pruning')
    parser.add_argument('--prune_interval', type=int, default=10,
                        help='Interval (in epochs) to apply pruning')

    args = parser.parse_args()  # 解析 args 参数

    _init_(args)  # 调用 _init_ 函数时传递 args 参数
